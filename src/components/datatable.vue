    ...
    <div style="overflow: auto; width: 100%;" :style="tableStyle" :class="tableClass">
        <slot >
            <table style="table-layout: auto; min-width: 100%;" :class="{ 'dense': dense, 'divider': !notDivided, ...tableClass }" :style="tableStyle">
                <DatatableHeaders v-if="!hideHeader" is="head" 
...
const getColumns = computed<DatatableColumn[]>(() => {
    ...
    for (let column of columns.value) {
        ...
        if (newColumn.expansion && !("text" in newColumn.expansion)) newColumn.expansion.text = (val: any) => val
        if (!newColumn.id) newColumn.id = newColumn?.property?.replace(/[\W_]+/g, "_") || Math.random().toString(36).substring(2, 13)
