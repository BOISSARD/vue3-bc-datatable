...
<div style="max-width: 100%;"
    :class="{ 'theme--light': !dark, 'theme--dark': dark, 'pa-4 elevation-1 rounded table-wrapper': !nested }">

    <div v-if="$slots.title || title" class="table-container-top">
        <slot name="title" :title="title" :rows="rows" :displaying="getRows"><span class="table-title">{{ title
        }}</span></slot>
    </div>
    ...
    <div style="overflow: auto; width: 100%;" :style="tableStyle" :class="tableClass">
        <slot v-bind="getThis">
            <table style="table-layout: auto; min-width: 100%;"
                :class="{ 'dense': dense, 'divider': !notDivided, ...tableClass }" :style="tableStyle">
                <DatatableHeaders v-if="!hideHeader" :style="{ ...getSticky('thead') }" is="head" :table="getThis"
                    :identifiant="identifiant" :columns="getColumns" :displaying="getRows" :debug="debug"
                    :select="selecting">
                    <template v-for="slotName of Object.keys($slots).filter(name => name.startsWith('header'))"
                        :key="slotName" #[slotName]="slotProps">
                ...
                <DatatableHeaders v-if="displayFooter" :style="{ ...getSticky('tfoot') }" is="foot" :table="getThis"
                    :identifiant="identifiant" :columns="getColumns" :displaying="getRows" :debug="debug"
                    :select="selecting">
                    <template v-for="slotName of Object.keys($slots).filter(name => name.startsWith('footer'))"
                        :key="slotName" #[slotName]="slotProps">
                        <slot :name="slotName" v-bind="slotProps" />
                    </template>
                </DatatableHeaders>
...
import { StyleProps, ClassProps, DatatableColumn, DatatableRow, DatatablePropertyId, DatatableSort, DatatableColumnSort, DatatableGroup, DatatableFilter, DatatableExpansion, DatatableSelection, DatatableSticky, DatatablePagination, DatatableDraggable, DatatableResizable, DatatableColumnSelection } from './types';
...
watch(() => props.columns, () => {
    // console.info(`${props.identifiant} watch props.columns :`, props.columns, isRef(props.columns), isReactive(props.columns))
...
const getColumns = computed<DatatableColumn[]>(() => {
    ...
    for (let column of columns.value) {
        ...
        if (column.expansion && !("text" in column.expansion)) column.expansion.text = (val: any) => val
        if (!column.id) column.id = column?.property?.replace(/[\W_]+/g, "_") || Math.random().toString(36).substring(2, 13)
    }
...
const getRows = computed(() => {
    ...
    retour.sort((s1: Partial<DatatableRow>, s2: Partial<DatatableRow>) => {
        ...
        for (let s of sorting.value) {
            let r1 = s1[s.column], r2 = s2[s.column]
            ...
            if (col) {
                if (typeof col.sort == "boolean") {
                    if (typeof r1 === "string" && typeof r2 === "string")
                        sortResult = r1.localeCompare(r2, 'en', { sensitivity: 'base', numeric: true, usage: 'sort', ignorePunctuation: true })
                    else sortResult = r1 > r2 ? 1 : r1 < r2 ? -1 : 0
                } else if (typeof col.sort == "function") {
                    sortResult = col.sort(r1, r2)
                }
            }
            if (sortResult === undefined) throw new Error(`Sort method ${col.sort} invalid for column ${col.id}`)
...
function format(column: DatatableColumn, rowOrRows: DatatableRow | DatatableRow[], position: 'body' | 'header' | 'footer' | 'expansion' = "body") {
    ...
    switch (position) {
        ...
        default:
            ...
            args.push(((rowOrRows?.value) ?? rowOrRows).map?.((row: DatatableRow) => row[column?.property]))
            args.push(rowOrRows)
    }

    // console.log(`${props.identifiant} format :`, args, formating?.text(...args))
...
watch([() => props.sort, () => props.multiSort], ([newSort, newMultiSort]: [DatatableSort, boolean], [oldSort, oldMultiSort]: [DatatableSort, boolean]) => {
...
function updateSort(column: DatatableColumn, action: string) {
    ...
    let foundIndex = sorting.value.findIndex((col: DatatableColumnSort) => col.column === column.id)
    if (foundIndex === undefined) return

    if (action == "position") { // Si on souhaite changer la position de la colonne dans le sort
        // console.log(`${this.identifiant}`,"change position", foundIndex, this.sorting.length)
        sorting.value.splice((foundIndex + 1) % sorting.value.length, 0, sorting.value.splice(foundIndex, 1)[0]);
    } else { // Si on souhaite modifier l'utilisation de la colonne dans le sort 
        // console.log(`${this.identifiant}`,"change sort", foundIndex)
        if (foundIndex < 0) { // Si La colonne n'est pas utilisée
            if (props.multiSort) sorting.value.push({ column: column.id, desc: false })
            else sorting.value.splice(0, sorting.value.length, { column: column.id, desc: false })
        } else { // Si la colonne est déjà utilisée dans le sort 
            let found = sorting.value[foundIndex]
            if (!found.desc) sorting.value.splice(foundIndex, 1, Object.assign(found, { desc: true })) //found.desc = true // Et que le sens est ascendant alors on passe en descendant
...
function getExpandedValue(column, row) {
    ...
    return expandedRows.length == 0 ? -1 : expandedRows.length == Object.keys(getExpanded.value[column?.id]).length ? 1 : 0
}
...
async function expanse(fold, column, row) {
    ...
    if (row) {
        ...
        if (fold) {
            ...
            if (single)
                expand.value = expand.value.filter(r => !r.match(new RegExp(`col-${column.id}--row-.*`)))
            if (!expand.value.includes(id))
                expand.value.push(id)
        }
    } else {
        ...
        for (let row of getRows.value) {
            ...
            if (fold) {
                ...
                if (!expand.value.includes(id))
                    timeouts.push(new Promise((resolve) => {
                        setTimeout(() => {
...
watch(() => props.select, () => {
    if (Array.isArray(props.select)) {
        selecting.value = selectIsOnlyOfStrings(props.select) ? cloneDeep(props.select) : props.select.map(selection => Object.assign(new DatatableColumnSelection(), cloneDeep(selection)))
    } else {
        selecting.value = []
    }
}, { deep: true, immediate: true })
...
function selectIsOnlyOfStrings(select: DatatableSelection): select is string[] {
    return select?.reduce((isString: boolean, selection: DatatableColumnSelection | string) => isString && typeof selection === "string", true)
}
...
function getSelect(column: DatatableColumn) {
    if (selectIsOnlyOfStrings(selecting.value)) {
        ...
        return selecting.value.find((selection: DatatableColumnSelection) => selection.column === column.id)?.selected
    }
}
...
function selectAll(column: DatatableColumn, event: Event) {
    ...
    if (selectIsOnlyOfStrings(selecting.value)) {
        retour = (event.target as HTMLInputElement).checked ? getRows.value.map((row: Partial<DatatableRow>) => getId(row)) : []
    } else {
        let indexSelection = (selecting.value as Array<DatatableColumnSelection>)?.findIndex((selection: DatatableColumnSelection) => selection.column === column.id)
        let selection: DatatableColumnSelection = indexSelection >= 0 ? selecting.value[indexSelection] : Object.assign(new DatatableColumnSelection(), { column: column.id, selected: [] } as DatatableColumnSelection)
        selection.selected = (event.target as HTMLInputElement).checked ? getRows.value.map((row: Partial<DatatableRow>) => getId(row)) : []
        retour.splice(indexSelection >= 0 ? indexSelection : selecting.value.length, indexSelection >= 0 ? 1 : 0, selection)
    }
...
function select(column: DatatableColumn, row: DatatableRow, event: Event) {
    ...
    if (selectIsOnlyOfStrings(selecting.value)) {
        ...
        if (index < 0)
            selecting.value.push(getId(row))
...
function getSticky(position: "tfoot" | "thead" | DatatableColumn, zIndex: number | null = null) {
    let retour: { position?: string, bottom?: number | string, top?: number | string, right?: number | string, left?: number | string, zIndex?: number } = {}
    ...
    if (position == "tfoot" && props.stick?.footer) {
        ...
        if (typeof position.sticky === "string") {
            ...
            if (position.sticky.position) retour[position.sticky.position] = position.sticky.distance ?? 0
...
const whatPropId = computed(() => {
    ...
    else if (typeof props.propId === "object" && props.propId?.propId) return props.propId.propId
...
table {
    ...
    td {
        font-size: .8rem;
        transition: padding .2s cubic-bezier(.4, 0, .6, 1);
        padding: 6px 16px;
        // padding: 0 16px;
        height: 38px;
